...
const isNumberValid = (inputString, {precision = 9, scale = 0}) => {
    var perc = inputString.split(".").join("").length;
    var check  = false;
    var scal;
    var float = parseFloat(inputString);
    var int = parseInt(inputString);
    var arrLength = inputString.split(".").length

    if(arrLength == 2 && !isNaN(inputString) && !isNaN(float)){

        scal = inputString.split(".")[1].length;

    }else if(arrLength == 1 && !isNaN(inputString) && !isNaN(int)){

        scal = 0;

    }
    if(perc <= precision && scal <= scale){
...
utils.test(isNumberValid("10.0",{scale: 0}), false); // number valid, but scale too low
